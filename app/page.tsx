...
export default function App() {
  ...
  const select = (id: string) => {
    ...
    props = { items, categories, config, notify, reload: load, setReceipt };
  return (
...
function Billing({ items, config, notify, setReceipt }: any) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      const result = await api("/api/invoices", { method: "POST", body: JSON.stringify({ customer_name: customer.name, customer_phone: customer.phone, customer_place: customer.place, items: lines, discount: discountAmount, tax_rate: amount(tax), advance_id: selected?.id }) });
  ...
  const remove = async (row: any) => {
    if(confirm("Delete this invoice? If it had an advance applied, the advance will be restored.")) {
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "create" ? (
        ...
        <DataTable endpoint="/api/invoices" reloadTrigger={reloadTrigger} columns={[{key:"invoice_number",label:"Inv #"},{key:"created_at",label:"Date",render:(r:any)=>r.created_at?.slice(0,10)},{key:"customer_name",label:"Customer"},{key:"grand_total",label:"Total",render:(r:any)=>rupees(r.grand_total)}]} onEdit={viewReceipt} onDelete={remove} />
      )}
    </div>
  );
}
function AdvanceForm({ notify, setReceipt }: any) {
  const [tab, setTab] = useState("create");
  const [f, setF] = useState<any>({ id: "", customer_name: "", customer_phone: "", customer_place: "", advance_amount: "", notes: "" });
  const [reloadTrigger, setReloadTrigger] = useState(0);
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      setF({ id: "", customer_name: "", customer_phone: "", customer_place: "", advance_amount: "", notes: "" });
      setReloadTrigger(x => x + 1);
      setTab("manage");
    } catch (e: any) { notify(e.message); }
  };
  const edit = (row: any) => { setF({ id: row.id, customer_name: row.customer_name, customer_phone: row.customer_phone || "", customer_place: row.customer_place || "", advance_amount: row.advance_amount, notes: row.notes || "" }); setTab("create"); };
  const remove = async (row: any) => { if(confirm("Delete advance receipt?")) { await api("/api/advances", { method: "DELETE", body: JSON.stringify({ id: row.id }) }); setReloadTrigger(x => x + 1); notify("Advance deleted."); } };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-4 border-b pb-4">
        ...
        <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
          <button type="button" className={`text-sm px-4 py-1.5 rounded-lg font-medium transition-colors ${tab==="create"?"bg-white text-brand-700 shadow":"text-slate-600 hover:text-slate-900"}`} onClick={()=>{setTab("create"); setF({ id: "", customer_name: "", customer_phone: "", customer_place: "", advance_amount: "", notes: "" });}}>Create Advance</button>
          <button type="button" className={`text-sm px-4 py-1.5 rounded-lg font-medium transition-colors ${tab==="manage"?"bg-white text-brand-700 shadow":"text-slate-600 hover:text-slate-900"}`} onClick={()=>setTab("manage")}>Manage</button>
        </div>
      </div>
      {tab === "create" ? (
        ...
        <DataTable endpoint="/api/advances" reloadTrigger={reloadTrigger} columns={[{key:"receipt_number",label:"Receipt #"},{key:"issued_at",label:"Date",render:(r:any)=>r.issued_at?.slice(0,10)},{key:"customer_name",label:"Customer"},{key:"advance_amount",label:"Amount",render:(r:any)=>rupees(r.advance_amount)}]} onEdit={edit} onDelete={remove} />
      )}
    </div>
  );
}
function Inventory({ items, reload, notify }: any) {
  ...
  const remove = async (row: any) => { if(confirm("Archive item?")) { await api("/api/catalog/items", { method: "DELETE", body: JSON.stringify({ id: row.id }) }); reload(); setReloadTrigger(x => x + 1); notify("Item archived."); } };
...
function Expenses({ categories, reload, notify }: any) {
  ...
  const remove = async (row: any) => { if(confirm("Delete expense?")) { await api("/api/expenses", { method: "DELETE", body: JSON.stringify({ id: row.id }) }); setReloadTrigger(x => x + 1); notify("Expense deleted."); } };
      ...
      {tab === "create" ? (
        <div className="space-y-4">
          ...
          <div className="card">
            ...
            <div className="mt-3 flex flex-wrap gap-2">{categories.map((x: any) => <span className="rounded-full bg-brand-50 px-3 py-1 text-sm text-brand-700" key={x.id}>{x.name}</span>)}</div>
          </div>
        </div>
      ) : (
        ...
        <div id="receipt" className="rounded-2xl bg-white p-6 shadow-2xl">
          <div className="text-center">
            ...
            <p className="font-semibold">
              {data.type === "advance" ? "ADVANCE RECEIPT" : "TAX INVOICE"}
            </p>
            <p className="text-sm">
          ...
          <div className="mt-4 space-y-1 text-sm">
            ...
            <p className="flex justify-between text-lg font-bold">
              ...
              <span>{rupees(data.advance_amount || data.grand_total)}</span>
            </p>
...
function DataTable({ endpoint, columns, onEdit, onDelete, filterDate = true, reloadTrigger = 0 }: any) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setData(res.items || res); // handle both paginated and direct array
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
              ...
              {data.map((row, i) => (
                <tr key={row.id || i} className="hover:bg-slate-50">
                  ...
                  <td className="p-3 flex gap-3">
                    ...
                    {onDelete && <button type="button" onClick={() => onDelete(row)} className="text-rose-600 font-medium hover:underline">Delete</button>}
                  </td>
                </tr>
              ))}
            </tbody>
