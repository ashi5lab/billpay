...
// 1. Remove existing function DataTable if it's at the end
const dataTableIndex = text.indexOf('function DataTable(');
if (dataTableIndex !== -1) {
  text = text.substring(0, dataTableIndex);
}

// 2. We need to add Modal, ConfirmModal, and new DataTable at the end of the file
...
// 3. Fix ReceiptView (Invoice Modal)
...
// 4. Remove confirm() from Billing, AdvanceForm, Inventory, Expenses if they still have it
...
// 5. Replace expensesNew fully with the Modal version for categories!
function replaceFunction(funcName, replacementCode) {
  const start = \`function \${funcName}(\`;
  ...
  if (idx !== -1) {
    ...
    for (let i = idx; i < text.length; i++) {
       if (text[i] === '"' || text[i] === "'" || text[i] === '\`') {
         if (!escape) {
           ...
           else if (inString === text[i]) inString = false;
         }
       }
       if (text[i] === '\\\\') escape = !escape;
       else escape = false;
    ...
    if (endIdx !== -1) {
       text = text.substring(0, idx) + replacementCode + "\\n" + text.substring(endIdx);
    }
  }
}
...
replaceFunction('Expenses', expensesNew);

fs.writeFileSync('app/page.tsx', text);
console.log('Update page.tsx script executed successfully');
